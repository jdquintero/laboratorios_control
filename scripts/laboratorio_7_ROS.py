...
class NodePosition():
    ...
    def initParameters(self):
        ...
        self.kd = 1
	self.j = 0
	self.xd = (5 * cos(j)) + (1 * cos(4 * j) )
	self.yd = (5 * sen(j)) + (1 * sen(4* j) )
        self.xe_prev = 0
        self.v_limit = 0.5
	self.inici = 0
	...
	self.ini = self.rospy.Subscriber(self.topic_ini, double, self.callback_ini)
    ...
    def controller(self):
        self.xe = self.xd(j+1) - self.xr 
        self.ye = self.yd(j+1) - self.yr
	if xe<0.1 and ye <0.1:
		self.e = [[self.xe],
		          [self.ye]]
		self.K = [[1, 0],
		          [0, 1]]
    ...
    def main(self):
        ...
        while not self.rospy.is_shutdown():
            if self.change_odom and  self.change_ini:
                self.controller()
                self.makeVelMsg()
                self.makeErrorMsg()
